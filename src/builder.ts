...
const echoAgent = AgentBuilder()
  .text(({ content }) => content ?? "No content provided")
  .createAgent({
    agentCard: "EchoAgent",
  });
const testAgent = AgentBuilder()
  .text(() => "You have successfully reached the test agent")
  .createAgent({
    agentCard: "TestAgent",
  });
...
const aiagent = AIAgentBuilder(
  {
    ...
    apiKey:
      "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",
  },
  {
    callerId: "openai-agent",
    agents: new Map([
  ...
  .ai("Always tell the user what type of tools you have available to you.")
  .createAgent({
    agentCard: "OpenAiAgent",
  });

await aiagent
  .sendMessage(
    "Check if you have access to the test agent and send a message to it, then tell me what the test agent says."
  )
  .then((result) => {
